"""
EEG Unified Scope - 多设备脑电波形显示 (示波器模式)
与 roll 模式的区别：光标从左到右扫描，满屏后清空重绘，模仿示波器行为。
"""
...
# 【SCOPE】时间轴从左到右 0→SHOW_SECONDS
time_axis = np.linspace(0, SHOW_SECONDS, PLOT_POINTS)
...
def parse_eeg_packet(data_bytes: bytes, cfg: dict):
    ...
    if sp_row == 1:
        data_1ch = data.reshape(-1, 1)
        return np.append(data_1ch, data_1ch, axis=1)
    else:
        return data.reshape(n_samp // sp_row, sp_row)
...
class EEGViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EEG Unified Scope - 示波器模式")
        ...
        self.cursors = [0] * self.n_channels  # 每个通道独立光标
    ...
    def _setup_ui(self):
        ...
        #     QComboBox {
        #         background-color: #2a2a4a; color: #e6e6e6;
    ...
    def _rebuild_plots(self):
        ...
        for i in range(self.n_channels):
            ...
            # 【SCOPE】X 轴 0→SHOW_SECONDS
            pw.setXRange(0, SHOW_SECONDS)
    ...
    def _on_device_changed(self, idx):
        ...
        self.eeg_buffers = np.zeros((self.n_channels, PLOT_POINTS))
        self.cursors = [0] * self.n_channels
        self.filters.clear()
        ...
        self.ble_worker.set_config(self.cfg)
        self._rebuild_plots()
        self.setWindowTitle(f"EEG Unified Scope - {self.cfg['name']}")
    ...
    def _on_eeg_data(self, data):
        """
        Scope 写入逻辑：
        每个通道独立光标，新数据写入光标位置 → 光标前进
        光标到达末尾 → 归零从头覆盖，不清空旧数据
        """
        n = data.shape[0]
        if n == 0:
            return
        ...
        for ch in range(self.n_channels):
            cursor = self.cursors[ch]
            write_len = min(n, PLOT_POINTS - cursor)
            self.eeg_buffers[ch, cursor:cursor + write_len] = filtered[:write_len, ch]
            cursor += write_len

            # 还有剩余数据未写入（光标已到末尾）
            if cursor >= PLOT_POINTS:
                remaining = n - write_len
                cursor = 0  # 归零从头覆盖，不清空旧数据
                if remaining > 0:
                    self.eeg_buffers[ch, :remaining] = filtered[write_len:, ch]
                    cursor = remaining

            self.cursors[ch] = cursor
